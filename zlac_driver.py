...
class ZLACDriver:
    ...
    def clear_fault(self):
        """Send fault reset command to ZLAC (0x0006 to control word)."""
        cmd_body = struct.pack('>BBHH', self.modbus_id, self.WRITE, self.CONTROL_WORD, 0x0006)
        result = self._send_command(cmd_body + self._calculate_crc(cmd_body), 8)
        time.sleep(0.1)
