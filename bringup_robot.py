...
class VicPinky(Node):
    ...
    def __init__(self):
        ...
        self.get_logger().info("2. Clearing fault & resetting motor state...")
        self.driver.clear_fault()
        self.driver.disable()
        time.sleep(0.3)

        self.get_logger().info("3. Setting velocity mode...")
        ...
        self.get_logger().info("4. Enabling motors...")
    ...
    def twist_callback(self, msg: Twist):
        ...
        try:
            ...
            rpm_l = max(min(int(rpm_l), 35), -35)
            rpm_r = max(min(int(rpm_r), 35), -35)
    ...
    def update_and_publish(self):
        ...
        if rpm_l is None or encoder_l is None:
            self._fail_count += 1
            self.get_logger().warn(f"Motor read failed (x{self._fail_count})")
            if self._fail_count >= 5:
                self.get_logger().error("Motor fault! Re-enabling...")
                self.driver.clear_fault()
